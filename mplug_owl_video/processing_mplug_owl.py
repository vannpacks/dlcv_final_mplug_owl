...
def load_video(path, num_frames, kfid):
    vr = VideoReader(path, height=224, width=224)
    frame_indices = kfid
    images_group = list()
    total_frames = len(frame_indices)
    # input_index = get_index(total_frames, num_frames)
...
class MplugOwlProcessor(ProcessorMixin):
    ...
    def __call__(self, text=None, images=None, videos=None, kfid = None, num_frames=4, return_tensors=None, **kwargs):
        ...
        if videos is not None:
            ...
            for id, video in enumerate(videos):
                video_frames = load_video(video, num_frames, kfid[id])
